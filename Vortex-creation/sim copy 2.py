...
# velocity[:,:,2] = u_in*np.fromfunction(lambda i, j: (abs(i-30)+abs(j-20))<10,(ny,nx)).astype(int)
# velocity[:,:,5] = u_in*np.fromfunction(lambda i, j: (abs(i-30)+abs(j-20))<10,(ny,nx)).astype(int)
# density = np.fromfunction(lambda i, j: (abs(i-30)+abs(j-20))<10,(ny,nx)).astype(int)
inlet_velocity = velocity

# plt.matshow(velocity[:,:,1])
# plt.show()

def f_equilibrium(W, v, rho):
    f_eq = np.zeros((ny,nx,9))
    v_sq = np.sum(v*v,axis=2)#np.linalg.norm(u, axis=2)*np.linalg.norm(u, axis=2)
    for i in range(9):
        # f_eq[:,:,i] = W[i]*rho*(1+3*u[:,:,i]+9/2*u[:,:,i]*u[:,:,i]-3/2*u_sq)
        f_eq[:,:,i] = W[i]*rho*(1+3*v[:,:,i]+9/2*v[:,:,i]*v[:,:,i]-3/2*v_sq)
    return f_eq
...
for st in range(250):
    ...
    density = funct_on_subset(setone,wedge_inside,density)
    # particle_distribution = funct_on_subset(setzero, np.repeat(wedge_inside[:,:, np.newaxis], 9, axis=2), particle_distribution)
    for i in range(9):
    ...
    collision_distribution = particle_distribution + (particle_distribution_eq - particle_distribution)/tau
    ...
    particle_distribution = particle_distribution/np.linalg.norm(particle_distribution)
